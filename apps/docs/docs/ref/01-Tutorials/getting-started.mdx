...
For this example, we will use SqlLite with the `@operativa/verse-sqlite` provider. Verse supports many database providers, including PostgreSQL, MySQL, and SQLite. You can find the list of supported providers in the [Database Providers](/docs/guides/database-providers) section.
...
```ts include ./apps/snippets/src/basic.ts:1-20 showLineNumbers
...
```ts include ./apps/snippets/src/basic.ts:38-38 showLineNumbers
...
```ts include ./apps/snippets/src/basic.ts:50-54 showLineNumbers
...
```ts include ./apps/snippets/src/basic.ts:56-66 showLineNumbers
